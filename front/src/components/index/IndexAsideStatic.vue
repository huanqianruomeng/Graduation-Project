<template>
  <el-aside class="index-aside" height="100vh" width="230px">
    <div class="index-aside-inner menulist" style="height:100%">
      <div v-for="item in menuList" :key="item.roleName" v-if="role==item.roleName" class="menulist-item" style="height:100%;broder:0;background-color:#2563eb">
        ...
        <el-menu mode="vertical" :unique-opened="true" class="el-menu-demo" style="height:100%;" background-color="#2563eb" text-color="#ffffff" active-text-color="#9AF4F4" default-active="0">
...
export default {
  ...
  let that = this
  this.$nextTick(()=>{
    // 处理普通菜单项 .el-menu-item
    document.querySelectorAll('.menulist .el-menu-item').forEach(el=>{
      el.addEventListener("mouseenter", e => {
        e.stopPropagation()
        el.style.backgroundColor = "rgba(133, 218, 231, 1)" // 统一蓝色悬浮色
      })
      el.addEventListener("mouseleave", e => {
        e.stopPropagation()
        el.style.backgroundColor = "#2563eb" // 导航栏默认更浅蓝色
      })
      el.addEventListener("focus", e => {
        e.stopPropagation()
        el.style.backgroundColor = "rgba(133, 218, 231, 1)" // 和悬浮色保持一致
      })
    })
    // 处理子菜单标题 .el-submenu__title
    document.querySelectorAll('.menulist .el-submenu__title').forEach(el=>{
      ...
      el.addEventListener("mouseleave", e => {
        e.stopPropagation()
      el.style.backgroundColor = "#2563eb" // 改为导航栏默认更浅蓝色
      })
    })
  })
},
